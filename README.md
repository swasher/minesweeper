...
[ ] Научить мышь двигаться, эмулируя движеня человека, для обхода анти-бот систем
[ ] Сделать один рандом-сольвер с параметрами - Выбирать угол, выбирать стенку, выбирать по вероятности 
[ ] Солверы должны возвращать не кнопку мыши, а предполагаемый статус ячейки - Mine, Empty. В еще более продвинутом
    варианте - каждая ячейка должна быть объектом, содержащим нашедший ее алгорит, расстояние до последнего клика (еще что??)
    И все найденные решения должны храниться всеми солверами добавляются в List. Затем выбирается наиболее подходящая
    ячейка (по расстоянию, например) и рассчитывается время думания (по цифрам вокруг ячейки и сработавшему алгоритму)
[ ] Рефактор: так как у нас теперь ячейка "знают" о матрице, переместить методы around* из матрицы в ячейки. Тогда, 
    например, в Tk, вместо self.matrix.get_around_flagged(cell) можно писать cell.get_around_flagged()
[ ] Переделать свойства region_x1 и аналогичные в объекты Point
...
- сначала импортириуем весь модуль `import asset`
- из него вызываем `asset.init('asset_tk')`, который инициализирует объекты
- и уже затем импортируем все заново инициализированные объекты с новым путем - `from asset import *`

Account block resistance
========================

Для противодействия блокировке аккаунта используется комплекс мер, делающий игру неотличимой от реального человека:

#### Нажатие и отпускание кнопки мыши

По моим замерам, если *стараться* быстро кликнуть, то получается около 0,02-0,05s. При спокойной и размеренной игре
