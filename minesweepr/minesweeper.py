...
import queue
import functools
from .util import ImmutableMixin, listify, map_reduce, peek, graph_traverse, fact_div, choose, product
...
class PermutationSet(object):
    ...
    def __init__(self, cells_, k, permus):
        ...
        permus -- set of 'Permutation's thereof; all permutations must share
            the same cell set and # of mines! (corresponding to 'cells_' and 'k')
        """
        self.cells_ = cells_
        self.k = k
        self.permus = permus
...
class FrontTally(object):
    ...
    def __iter__(self):
        # todo swasher
        #   Обернул в iter, потому что self.totals.items() не возвращает итератор.
...
def combine_fronts(tallies, num_uncharted_cells, at_large_mines):
    ...
    class FrontPerMineTotals(object):
        ...
        def multiply(self, n):
            ...
            # todo swasher
            #   как и ранее, заменил self (подразумевающий тут self.__iter__()) на явно написанный self.totals.items()
            return FrontPerMineTotals(dict((num_mines, n * count) for num_mines, count in self.totals.items()))
    ...
    # todo by swasher
    #   я обернул map(CombinedFront.from_tally, tallies) в list, потому что нельзя сложить map и list
