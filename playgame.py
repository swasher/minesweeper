"""
Как я думаю сделать... playgame должен запускаться из другого процесса python,
и загружать некую матрицу из pickle, и отображать валидное поле Vienna,
на котором и будем тестить алгоритмы
"""
from tkinter import *
...
assetdir = 'asset_vienna_full'
test_game = 'game_SAVE_24-Nov-2021--19.30.21.082856'

matrix = load(test_game)

# todo rename W and H
SIZE_X = matrix.height
SIZE_Y = matrix.width
...
class Minesweeper:
    ...
    def __init__(self, tk):
        ...
        # import images
        self.images = {
            "plain": PhotoImage(file = "asset_testing/tile_plain.gif"),
            "clicked": PhotoImage(file = "asset_testing/tile_clicked.gif"),
            "mine": PhotoImage(file = "asset_testing/tile_mine.gif"),
            "flag": PhotoImage(file = "asset_testing/tile_flag.gif"),
            "wrong": PhotoImage(file = "asset_testing/tile_wrong.gif"),
            "numbers": []
        }
        for i in range(1, 9):
            self.images["numbers"].append(PhotoImage(file = "asset_testing/tile_"+str(i)+".gif"))
